		...
		{#if message.id !== -1}
			<div use:clickOutside={() => (isOptionsExpanded = false)}>
				...
				{#if isOptionsExpanded}
					<div
						class="absolute top-14 z-50 grid w-max rounded-[1rem] bg-white/90 p-2 shadow-lg shadow-primary-900/20 backdrop-blur dark:bg-primary-950/90 dark:shadow-black/30 {message.role ===
						'user'
							? 'left-0 -ml-2'
							: 'right-0 -mr-2'}"
