...
>
	<h1
		class="relative grid h-11 bg-blue-600 bg-gradient-to-tr from-blue-700 to-blue-500 bg-clip-text text-2xl font-black text-transparent lg:px-2"
	>
	...
	{#if $page.data.session}
		<a
			...
			href="https://checkout.kal.chat/checkout/buy/77494bec-e48e-4193-b08d-fb5816326a1f"
		>
			<div class="text-lg">✨</div>
			<div class="pr-2">Subscribe</div>
		</a>
		<a
			class="pointer-events-auto my-auto flex flex-shrink-0 items-center justify-center gap-2 rounded-[1.75rem] bg-white/90 py-1 px-1 text-blue-600 transition-all duration-150 hover:bg-white/95 hover:shadow hover:shadow-blue-600/10 focus:bg-white/95 active:bg-white/75 active:shadow-none sm:pr-4"
			href="/account"
			title={$page.data.session.user?.email}
		>
			{#if $page.data.session.user?.image}
				<img
					class="h-[2.25rem] w-[2.25rem] rounded-full bg-white/90 object-cover shadow shadow-blue-900/10"
					alt=""
	...
	{:else if $page.url.pathname !== '/account'}
		<a
			class="pointer-events-auto flex flex-shrink-0 items-center justify-center gap-2 rounded-[1.75rem] bg-white/90 py-2 px-4 text-sm font-semibold uppercase text-blue-600 transition-all duration-150 hover:bg-white/95 hover:shadow hover:shadow-blue-600/10 focus:bg-white/95 active:bg-white/75 active:shadow-none"
