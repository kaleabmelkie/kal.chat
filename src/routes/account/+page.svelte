			...
			<div class="grid gap-2">
				...
				<span class="text-lg font-semibold text-primary-700/90 dark:text-primary-300/90">
					{@html data.session.user.plan === 'free'
						? '<strong>Free</strong> user (faster model only) &nbsp;👀'
						: data.session.user.plan === 'paid'
							? '<strong>Pro</strong> user (faster or better model) &nbsp;🎉'
							: data.session.user.plan === 'grace'
								? '<strong>Grace</strong> period (faster or better model)<br/>Downgrading to free user soon &nbsp;🫤'
								: data.session.user.plan === 'sponsored'
									? '<strong>Sponsored</strong> user (faster or better model)<br/>Paid for by Kaleab &nbsp;🎁'
